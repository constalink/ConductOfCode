...
here. Each type of method has different requirements, restrictions, etc. There are exceptions but those exceptions are
rare and will be carefully reviewed before accepted into the code base. 

A common exception is when overriding a parent method of a third party library that doesn't fit into any of these
categories. 

Another is when a language requires a certain method that doesn't fit into any of these categories. For example, the
Python `threaded` module requires a `run` method that starts the thread. The method falls into the *do method*
category, but doesn't follow naming conventions. In these cases, there is no alternative but to name the method `run`.
...
initialized and have proper values before returning. They are also responsible for calling a *designated init method*
from the immediate super class so that initialization can continue up the inheritance hierarchy. For the most part
a class should have a single *designated init method*. It is possible to have multiple *designated init methods*, but
you must make sure that no mater what designated init method is called, the object ends up in a valid state after
return. Here are a list of rules for designated init methods:
...
and your class might not have any *convenience init methods*. If your class does define *convenience init methods*, it
can have many of them depending on how you want to initialize your class. Create *convenience init* methods as a
shortcut, a common use case, or perhaps to convert or manipulate the input into the correct format for a *designated
init method*.

For example: A `Date` object may have a *designated init method* that takes a year, month, and day. You may also have a
*convenience init method* that takes a date string in the format of `YYYY-MM-DD`. The *convenience init method* would
then convert the string into a year, month, and day, then pass that data to the *designated init method*. Here are a
list of rules for *convenience init methods*.

* Must call another *init method* from the **same** class. This can either be another *convenience init method* or a
...
*Designated init methods* and *convenience init methods* have a delicate relationship and must follow certain rules in
order to ensure initialization consistency. Here are a list of rules that your *init methods* must follow:
...
* A *convenience init method* MUST ultimately call a *designated init method* from the **SAME** class, not from a super
  class. (Only *designated init methods* may call a *designated init method* from a super class... in fact, they have
  to)
...
Wait.... we're not done. There are a few other things to consider with regards to your *init methods*.

* No object should be used at all until it is initialized via an *init method*.
...
* Abstract or base classes may not have any public *init methods*. All their *init methods* must be protected and may
...
* *Init methods* with no parameters must be named `init` if public or `_init` if protected. If they have at least 1
  parameter, they must start with `initWith` or `_initWith` and should end with the name of the first parameter. Here
  are some examples (Written in pseudo code)
...
* For the most part, all *init methods* should return the instance itself. This allows for easy object creation and
  usage. Consider this code (Written in pseudo code)
...
* **Call** - Must call another *init method* until a base *designated init method* is called. May call *validate
  methods* to validate values before returning. May not call any other methods.

* **State** - *Designated init methods* MUST initialize object state. *Convenience init methods* MAY initialize object
  state **after** a *designated init method* is called.
...
* Must be named descriptively after what it is they return along with the name of the first parameter of the method.
  ...
  *give methods* in place of computed properties) then the method should just be named after what it returns. Rather
  than describing the naming rules in detail, we are just going to give you some signature examples using pseudo code:
...
* **Naming convention** - Must start with a noun, followed by a preposition, then the name of the first parameter. In
  the case that *give methods* must be used in place of computed properties, the name must be a noun. The noun should
  represent the return value of the method.

* **Parameters** - Must take 1 or more parameters in most cases. In the case that *give methods* must be used in place
  of computed properties, *give methods* may take no parameters.
...
the value of. You should call *validate methods* from your *init methods* before returning to validate values as well as
from any method that updates the state of the object. *Validate methods* are here to make sure that your objects are
...
* Must start with the word `validate` (or `_validate` if protected) and end with the name of the first parameter. In
  ...
  other words in between in order to make the name more descriptive of what it actually validates. Here are some 
  examples:
...
* *Validate methods* MUST throw an exception if the input cannot be validated or is invalid for any reason. The
  exception should (but doesn't have to) contain an error message that explains why validation failed. This error
  message can be extracted from the exception in order to be logged or to be displayed to the user if required.
...
The rules for do methods are pretty simple. They must start with the word `do` (or `_do` if protected), must end with
the name of the first parameter if there is one, and must never return a value. *Do methods* are the *engine and gears*
of your class. Call *do methods* to *do* something. If you want to load data or save data to an external source, use
